Attribute VB_Name = "图表标题_3_图表去除人工编号"
Option Explicit
...
' 图片标题手工编号清理（循环 + 进度条窗体）
' 规则：
'   （一）仅处理“正文”Story中、段首（经预清理后）以“图”开头的段落
'   （二）排除位于表格内的段落（避免表格中的图片说明被改动）
'   （三）Step A：从“图”删到“第一个中文字符”为止
'   （四）Step B（兜底）：正则清“图[可选连字符]数字[点分]*[可选-数字]”
'   （五）每轮结束：若已清除>0，询问是否继续；=0 则提示“清除完毕”
'   （六）进度与日志：沿用 ProgressForm
' ==========================================================

Public Sub 清除图片题手工编号_使用进度窗体()
    Const 仅处理指定样式 As Boolean = True
    Const 图题样式名 As String = "图片标题"
    Const 排除表格内 As Boolean = True
    ...
    On Error Resume Next
    Application.UndoRecord.StartCustomRecord "图片标题前缀清除（循环）"
    On Error GoTo 0
    ...
    Do
        ...
        '（三）统计候选数量
        Dim cand As Long
        cand = 统计候选段数_图(useStyleFilter, 图题样式名, 排除表格内)

        If cand = 0 Then
            progressForm.UpdateProgressBar 200, "本轮没有以“图”开头的候选段落。"
            MsgBox "图片标题前缀清除完毕（未发现候选）。" & vbCrLf & _
                   "累计已清除：" & allTouched & " 处。", vbInformation
            Exit Do
        End If

        '（四）执行单轮清理
        执行一轮清除_图 cand, useStyleFilter, 图题样式名, 排除表格内, total, touched, skipped
        allTouched = allTouched + touched
        ...
        If touched = 0 Then
            MsgBox "图片标题前缀清除完毕（本轮无可清除项）。" & vbCrLf & _
                   "累计已清除：" & allTouched & " 处。", vbInformation
            Exit Do
        Else
            ans = MsgBox("本轮已清除 " & touched & " 处前缀。" & vbCrLf & _
...
' 执行单轮清理（图片）——带进度条
' ----------------------------------------------------------
Private Sub 执行一轮清除_图(ByVal candTotal As Long, _
                        ByVal useStyleFilter As Boolean, _
                        ByVal targetStyleName As String, _
                        ByVal 排除表格内 As Boolean, _
                        ByRef total As Long, _
                        ByRef touched As Long, _
                        ByRef skipped As Long)
    ...
    For Each p In doc.Paragraphs
        ...
        If p.Range.StoryType <> wdMainTextStory Then GoTo NextPara
        If 排除表格内 Then
            If p.Range.Information(wdWithInTable) Then GoTo NextPara
        End If
        ...
        ' 候选：段首（强预清理后）以“图”开头
        oldTxt = 强预清理_段首(p.Range.text)
        If Len(oldTxt) = 0 Or Left$(oldTxt, 1) <> "图" Then GoTo NextPara
        ...
        ' Step A：从“图”删到首个中文
        newTxt = 去除图片题旧前缀_到第一个中文(r.text)

        ' Step B：正则兜底（图-1.2-3 之类）
        If newTxt = r.text Then
            newTxt = 正则替换(newTxt, _
                    "^\s*图\s*[-－–—]?\s*\d+(?:\s*[\.．。]\s*\d+)*\s*(?:[-－–—]\s*\d+)?(?:\s+\d+)?\s*", _
                    "")
            newTxt = 去噪直至首个中文(newTxt)
        End If
...
' ----------------------------------------------------------
Private Function 去除图片题旧前缀_到第一个中文(ByVal s As String) As String
    Dim i As Long, ch As String, hit As Boolean
    s = 强预清理_段首(s)
    If Len(s) = 0 Or Left$(s, 1) <> "图" Then
        去除图片题旧前缀_到第一个中文 = s
        Exit Function
    End If
    For i = 2 To Len(s)
